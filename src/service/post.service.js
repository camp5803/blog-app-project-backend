...
export const postService = {
    ...
    getCommentByPage: async (postId, page, pageSize, userId) => {
        try {
            let blockUser = [];
            if (userId) {
                blockUser = (await postRepository.getBlockedUser(userId)).map(obj => obj.blockUserId);
            }
            ...
            const rootCommentList = JSON.parse(JSON.stringify(await postRepository.getCommentList(filter, pagenation)));
            ...
            for (const comment of rootCommentList.rows) {
                ...
                comment.child = JSON.parse(JSON.stringify((await postRepository.getCommentList(filter)).rows));
            ...
            const totalResults = await postRepository.getCommentCount(postId);
